...
function makeUniqueError(): QueryFailedError {
  const err = new QueryFailedError(
    ...
    [],
    new Error(),
  ) as unknown as QueryFailedError & { code: string; detail: string };
  err.code = '23505';
  err.detail = 'Key (nickname)=(abc) already exists.';
  return err;
}

function makeDataSource(manager: Record<string, jest.Mock>) {
  return {
    transaction: jest.fn(
      (cb: (m: Record<string, jest.Mock>) => Promise<unknown>) => cb(manager),
    ),
